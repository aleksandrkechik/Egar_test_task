...
public class ExternalApproveService {
	...
	public int approveValuation(CollateralObject object) {
		if (object.getDate() == null || object.getValue() == null || object.getType() == null) {
			return -1;
		}
		if (object.getDate().isBefore(MIN_ASSESS_DATE))
			return (-2);
		switch (object.getType()) {
			case CAR:
				return object.getValue().compareTo(MIN_CAR_VALUE) < 0 ? -3 : 0;
			case AIRPLANE:
				return object.getValue().compareTo(MIN_PLANE_VALUE) < 0 ? -3 : 0;
			default:
				return  -100;
		}
	}
